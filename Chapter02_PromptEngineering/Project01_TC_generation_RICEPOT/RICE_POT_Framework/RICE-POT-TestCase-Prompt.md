...
A worked example of the RICE-POT prompt framework for generating enterprise-grade test cases from a PRD.
...
You are an expert QA Functional Tester with 15+ years of experience. You specialize in functional and non-functional testing and in writing enterprise-grade, traceable test cases.
...
Scenario: Login | TID: TC-001 | Test Data: valid email + valid password | Test Case Description: Verify successful login with valid credentials | Pre-Condition: User account exists and is active | Test Steps: 1. Open app.vwo.com 2. Enter valid email 3. Enter valid password 4. Click Login | Expected Result: User is redirected to the dashboard | Priority: High | Is Automated: No
...
- The anti-hallucination block (the "Don't" rules + the "Insufficient information" fallback) is what makes the output trustworthy for real QA work.
